...
if [ $nodo = 'node' ]
then
    rpmns_exec=$HOME/tiniba/$version/matrix_elements/rpmns.xeon
fi
if [ $nodo = 'itanium' ]
then
    rpmns_exec=$HOME/tiniba/$version/matrix_elements/rpmns.itanium
fi
if [ $nodo = 'quad' ]
then
    rpmns_exec=$HOME/tiniba/$version/matrix_elements/rpmns.quad
fi
if [ $nodo = 'hexa' ]
then
    rpmns_exec=$HOME/tiniba/$version/matrix_elements/rpmns.hexa
fi
##
if [ ! -d $input ]
then
...
# 
    ...
    printf "\t------------------------\n"
#    printf "\tWavefunction has ${MAG}finished ${NC} for: ${MAG}$node${NC} at $TIMEEND\n" 
