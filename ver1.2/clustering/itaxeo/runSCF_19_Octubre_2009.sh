...
#
 ...
 ABINITSquad=/home/prog/abinit/ABINITv4.6.5_QUAD/abinis
 ABINITPquad=/home/prog/abinit/ABINITv4.6.5_QUAD/abinip
# ABINITPhexa=/home/prog/HEXA/abinit-6.8.2/bin/abinit
 ABINITPhexa=/home/prog/apps/abinit-6.12.3-intel/64/bin/abinit
 INPUT1=$1
 MYOLDPWD=$PWD
...
if [ $CORRE == "1" ];then 
	...
	then 
	    ...
	    cd $DIR/$DIRSCF
	     ...
	     mpdtrace
	     mpiexec -ppn $NUCLEOS -n $nPROC -env I_MPI_DEVICE rdssm  $ABINITPquad < $CASO.files >&log
	     cd $DIR
	     Line
...
# gets the first hexa to launch the ring of the Nibelungs
	    ...
	    ssh $hexa0 "cd $DIR/$DIRSCF;mpiexec -ppn $NUCLEOS -n $nPROC -env I_MPI_DEVICE rdssm  $ABINITPhexa < $CASO.files >&log" 
	    Line
...
#;mpdtrace"
	    ...
	    ssh quad01 "cd $DIR/$DIRSCF;mpiexec -ppn $NUCLEOS -n $nPROC -env I_MPI_DEVICE rdssm  $ABINITPquad < $CASO.files >&log" 
	    Line
	    printf  "\tClosing the ring\n"
	    mpdallexit
	fi             
