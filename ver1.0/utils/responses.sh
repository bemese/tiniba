...
#	rm -f $sname.$ij.$spe'_'$pfix $sname.$ij.kk.$spe'_'$pfix hoy_$ij'_'$pfix
	...
	if [ $used_node == 'node' ]
	then
	    rsh ${nodes[$i]} "cd $dir;$laheylib  $where_latm/tetra_method_32b int_$ij'_'$pfix > hoy_$ij'_'$pfix" &
	elif [ $used_node == 'itanium' ]
	then
	    rsh ${nodes[$i]} "cd $dir;$laheylib  $where_latm/tetra_method_64b int_$ij'_'$pfix > hoy_$ij'_'$pfix" &
	elif [ $used_node == 'quad' ]
	then
#	    printf "\t$where_latm/tetra_method_quad int_$ij_$pfix > hoy_$ij_$pfix\n"
	    rsh ${nodes[$i]} "cd $dir;$laheylib  $where_latm/tetra_method_quad int_$ij'_'$pfix > hoy_$ij'_'$pfix" &
	else
