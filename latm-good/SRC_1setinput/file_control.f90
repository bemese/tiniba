    ...
    DO i=1,number_of_spectra_to_calculate
       IF (spectrum_info(i)%compute_integrand) THEN
          ...
          IF (istat/=0) THEN
             ...
             WRITE(6,*) 'file_control.f90: STOPPING2'
             STOP
!          ELSE IF (istat.EQ.0) THEN
!             if(debug) WRITE(6,*) 'File opened'
          END IF
       END IF
    END DO
  ...
  SUBROUTINE closeOutputDataFiles
    ...
    DO i=1,number_of_spectra_to_calculate
       IF (spectrum_info(i)%compute_integrand) THEN
          ...
          IF (istat.EQ.0) THEN
             if(debug) WRITE(6,*) 'Closing files'
          ELSE
             WRITE(6,*) ' '
             WRITE(6,*) 'Error closing', spectrum_info(i)%integrand_filename
             WRITE(6,*) 'Assigned unit', spectrum_info(i)%integrand_filename_unit
             WRITE(6,*) 'file_control.f90: STOPPING1'
             STOP
          END IF
       END IF
    END DO
  ...
  SUBROUTINE writeOutputDataFileHeaders
    ...
    IF (debug) WRITE(*,*) "Program Flow: entered file_control.f90"
...
!    rtmp = spin_factor*(1.d0/DFLOAT(2*nSym))
    ...
    DO i=1,number_of_spectra_to_calculate
       IF (spectrum_info(i)%compute_integrand) THEN
          ...
          if(debug) WRITE (6,*) "writeOutputDataFileHeaders", i
          if(debug) WRITE (6,*) "response-prefactor", rtmp2
          IF (debug) WRITE(*,*) "Program Flow: out of file_control.f90"
