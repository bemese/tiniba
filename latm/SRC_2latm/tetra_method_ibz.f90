...
! pp is the total contribution at one energy for one transition for one tetrahedron.
  CHARACTER(LEN=4) :: int2cha
  REAL*8 :: S1, S2, S3, S4
  INTEGER :: iTetra, iTrans, ic, iv
  INTEGER :: iostat, istat
  CHARACTER(LEN=120) :: outFile
  CHARACTER(LEN=6) :: charTemp
  ...
  WRITE(*,*) "Trying to allocated pp, size:", energy_Steps*nMax*nMax*16
  
  ALLOCATE( pp(energy_Steps,nMax*nMax), STAT=istat)
  WRITE(*,*) "istat = ", istat
  IF ( istat /= 0 ) THEN
    WRITE(*,*) "Could not allocate pp"
    STOP "Error allocating pp"     
!!!  ELSE
!!!     WRITE(*,*) "Allocated pp, size:", nTetra*energy_Steps*nMax*nMax*16
  END IF
...
! output
     !!!  add filename
     OPEN(85,FILE='temporaryFile')
     WRITE(85,*) iTetra
     CLOSE(85)
     OPEN(85,FILE='temporaryFile')
     READ(85,*) charTemp
     CLOSE(UNIT=85,STATUS='DELETE')
     outFile = TRIM(spectrum_filename) // '_' // TRIM(charTemp)
     WRITE(6,*) 'opening: ', outFile
     OPEN(77,FILE=outFile)
     DO ienergy=1, energy_Steps
        energy = energy_Out(ienergy)
